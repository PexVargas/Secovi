...
namespace PortalPexIM.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            filtro.Cidades = db.Imoveisclassificados.Where(x => x.SiglaEstado == siglaEstado &x.Excluido == 0).Select(x => x.Cidade).Distinct().OrderBy(x => x).ToArray();
            filtro.Bairros = db.Imoveisclassificados.Where(x => x.SiglaEstado == siglaEstado & x.Excluido == 0).Select(x => x.Bairro).Distinct().OrderBy(x => x).ToArray();
            filtro.Tipos = db.Imoveisclassificados.Where(x => x.SiglaEstado == siglaEstado & x.Excluido == 0).Select(x => x.Tipo).Distinct().OrderBy(x => x).ToArray();
